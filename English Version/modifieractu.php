<?php
  ...
  if(isset($_POST['submit'])){
    ...
    $du= mysqli_real_escape_string($conn, $_POST['du']);
    $au= mysqli_real_escape_string($conn, $_POST['au']);
    $message= mysqli_real_escape_string($conn, $_POST['message']);    

    $sql="update actu set id=$id,titre='$titre',du='$du',au='$au',message='$message' where id=$id";
    $result=mysqli_query($conn,$sql);
    if ($result) {
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h5 class="title">Editing a News</h5>
              </div>
              <div class="card-body">
                <form method="post">
                  <div class="row">
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group">
                        <label>From</label>
                        <input type="date" class="form-control" name="du" autocomplete="off" value="<?= htmlspecialchars($du) ?>">
                      </div>
                    </div>
                      <div class="col-md-6">
                      <div class="form-group">
                        <label>To</label>
                        <input type="date" class="form-control" name="au" autocomplete="off" value="<?= htmlspecialchars($au) ?>">
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        <label>Contents</label>
                         <textarea rows="6" cols="80" class="form-control" name="message" autocomplete="off"><?= htmlspecialchars($message) ?></textarea>
                      </div>
                    </div>
                    </div>
                  <div>
                <button type="submit" class="btn btn-fill btn-primary" name="submit">Edit</button>
                 </div>
                </form>
              </div>
            </div>
