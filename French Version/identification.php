...
if (isset($_POST['submit'])){
  $carte = stripslashes($_REQUEST['carte']);
  $carte = mysqli_real_escape_string($conn, $carte);
  $date = stripslashes($_REQUEST['date']);
  $date = mysqli_real_escape_string($conn, $date);
    $query = "SELECT * FROM `abonné` WHERE id='$carte' and naissance='$date'";
  ...
  if($rows==1){
    ...
    $message = "Le numero de carte ou la date de naissance est incorrect.";
  }
}
?>
...
<body>
        ...
        <div class="row px-xl-5">
            <div class="col-12">
                <nav class="breadcrumb bg-light mb-30">
                    ...
                    <span class="breadcrumb-item active">Identification</span>
                </nav>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid">
        <div class="row px-xl-5">
            <div class="col-lg-12">
                <h5 class="section-title position-relative text-uppercase mb-3"><span class="bg-secondary pr-3">Identifiez Vous</span></h5>
                <form method="post">
                <div class="bg-light p-30 mb-5">
                    <div class="row">
                        <div style="text-align: center; margin-right: auto; margin-left: auto;" class="col-md-3 form-group">
                            <label>Numero de Carte</label>
                            <input style="text-align: center;" class="form-control" name="carte" type="text" id="carte">
                        </div>
                    </div>
                    <div class="row">
                        <div style="text-align: center; margin-right: auto; margin-left: auto;" class="col-md-3 form-group">
                            <label>Date de Naissance</label>
                            <input style="text-align: center;" class="form-control" name="date" type="date" id="date">
                        </div>
                    </div>
                    <div class="row">
                        <button style="margin-left: auto; margin-right: auto;" type="submit" name="submit" class="btn btn-primary">S'Identifiez</button>
                    </div>
                    <div class="row">
                        <?php if (! empty($message)) { ?>
                        <p style="text-align: center; margin-right: auto; margin-left: auto;"><?php echo $message; ?></p>
                        <?php } ?>
                    </div>
                </div>
            </form>
